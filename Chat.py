...
# Function to set up the RAG (Retrieval-Augmented Generation) system
...
def query_rag(query, vector_dbs, top_k):
    ...
    return contents[:4000] if len(contents) > 4000 else contents
...
def main():
    ...
    if st.sidebar.button("Scrape and Add to Knowledge Base"):
        if not st.session_state.api_key:
            ...
            with st.spinner("Scraping websites in parallel..."):
                scraped_urls = scrape_urls_parallel(url_list, max_depth, min_content_length)
    ...
    if prompt := st.chat_input("Ask from your webpages..."):
        if 'vector_dbs' not in st.session_state or not st.session_state.vector_dbs:
            ...
            st.markdown(f"""<details><summary>Source</summary><p>{context}</p></details> """, unsafe_allow_html=True)
